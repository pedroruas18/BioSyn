...
class KnowledgeBase:
    ...
    def load_obo(self, kb):
        ...
        filepath = './resources/' + filepaths[kb] + '.obo'    
    ...
    def load_tsv(self, kb):
        ...
        filepath = "./resources/" + kb_dict[kb] + ".tsv"
        edges = list()
...
def generate_txt(kb_data):

    name_to_id = kb_data.name_to_id
    id_2_name_and_synonym = {}
    output = ''

    for term in name_to_id:
        kb_id = name_to_id[term]
        id_2_name_and_synonym[kb_id] = [term]

    kb_data.synonym_to_id

    for term in synonym_to_id:
        kb_id = synonym_to_id[term]
        id_2_name_and_synonym[kb_id].append(term)

    for kb_id in id_2_name_and_synonym:
        output += kb_id + '||'

        term_count = 0
        for name in id_2_name_and_synonym[kb_id]:
            output += name + '|'
            term_count += 1
    ...
    with open('resources/{}.txt'.format(kb_data.kb), 'w') as outfile:
